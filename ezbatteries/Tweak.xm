...
% ctor {
    if ([NSBundle.mainBundle.bundleIdentifier
         isEqual:@"com.apple.springboard"]) { // check if its springboard
        int regToken;                            // The registration token
        notify_register_dispatch(
                                 "com.idevicehacked.ezbatteries", &regToken,
                                 dispatch_get_main_queue(),
                                 ^(int token) { // Request notification delivery to a dispatch queue
                                     BCBatteryDeviceController *bcb =
                                     ...
                                     for (BCBatteryDevice *device in devices) {
                                         ...
                                         deviceCharge =
                                         MSHookIvar<long long>(device, "_percentCharge");
                                         
                                         confirmationAlertController.message =
                                         [confirmationAlertController.message
                                     ...
                                     [[UIApplication sharedApplication].keyWindow.rootViewController
                                      presentViewController:confirmationAlertController
                                      animated:YES
                                      completion:NULL];
                                 });
    }
